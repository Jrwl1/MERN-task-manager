...
const Task = () => {
  ...
  const [formData, setFormData] = useState({ // Initial form state now includes 'dueDate'
    description: "",
    dueDate: ""                                 
  });
    ...
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        ...
        setFormData({ description: data.task.description,
          dueDate: data.task.dueDate ? data.task.dueDate.split("T")[0] : "" }); // Convert ISO datetime to YYYY-MM-DD (only date part
      });
    }
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="mb-4">
                ...
                {/* This input uses the native HTML5 date picker */}
                <input
                type="date"                // tells browser to show a date picker
                name="dueDate"             // key in formData state
                id="dueDate"               // matches <label htmlFor>
                value={formData.dueDate}   // controlled component from state
                onChange={handleChange}    // uses the same generic handler
                className="w-full px-3 py-2 border rounded-md"  // tailwind styling
  />

                {/* If there's a validation error for dueDate, show it */}
                {fieldError("dueDate")}
              </div>
